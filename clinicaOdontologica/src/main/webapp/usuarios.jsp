...
<html lang="es-AR">
    ...
    <body>
                        ...
                        <table>
                            ...
                            <tbody>
                                <% for (User user : userList) {
                                        ...
                                        if ("Odontólogo".equals(user.getRole())) {
                                            roleKey = "dentist";
                                        } else if ("Secretario".equals(user.getRole())) {
                                            roleKey = "secretary";
                                        }
                                %>
                                <tr>
                                    <td class="mono" style="font-size:12px;"><%=user.getId()%></td>
                                    <td>
                                        <div class="row">
                                            <div class="avatar" style="background:var(--brand);color:var(--brand-ink);"><%=userInitials%></div>
                                            <div>
        ...
        <div class="scrim" data-modal-scrim="modal-usuario-edit"></div>
        <div class="modal" id="modal-usuario-edit">
            <div style="padding:20px 24px 6px;display:flex;align-items:center;justify-content:space-between;">
                <div>
                    <div style="font-family:var(--font-serif);font-size:22px;font-weight:500;">Editar usuario</div>
                    <div class="dim" style="font-size:12.5px;margin-top:2px;" data-edit-subtitle>Modificando usuario.</div>
                </div>
                <button class="btn btn-icon btn-ghost" data-modal-close aria-label="Cerrar">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
                </button>
            </div>
            <form action="EditUserSv" method="POST">
                <input type="hidden" name="id" data-edit-id />
                <div style="padding:18px 24px;">
                    <div class="grid-2">
                        <div class="field">
                            <label for="edit-username" class="field-label">Nombre de Usuario <span style="color:var(--danger);margin-left:3px;">*</span></label>
                            <input class="input" type="text" name="username" id="edit-username" required placeholder="marcelito" data-edit-username/>
                        </div>
                        <div class="field">
                            <label for="edit-password" class="field-label">Contraseña</label>
                            <input class="input" type="password" name="password" id="edit-password" placeholder="Dejar vacío para no cambiarla"/>
                        </div>
                        <div class="field">
                            <label for="edit-role" class="field-label">Rol <span style="color:var(--danger);margin-left:3px;">*</span></label>
                            <select class="select" id="edit-role" name="role" data-edit-role>
                                <option value="dentist">Odontólogo</option>
                                <option value="secretary">Secretario</option>
                                <option value="admin">Administrador</option>
                            </select>
        ...
        <div class="scrim" data-modal-scrim="modal-usuario-delete"></div>
        <div class="modal" id="modal-usuario-delete" style="width:min(420px,92vw);">
            <form action="DeleteUserSv" method="POST">
                <input type="hidden" name="id" data-delete-id />
                <div style="padding:22px 24px;">
                    <div class="row" style="gap:12px;margin-bottom:12px;align-items:flex-start;">
                        <div style="width:36px;height:36px;border-radius:10px;background:color-mix(in srgb, var(--danger) 14%, transparent);display:grid;place-items:center;color:var(--danger);flex-shrink:0;">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div style="flex:1;">
                            <div style="font-family:var(--font-serif);font-size:18px;font-weight:500;">Eliminar usuario</div>
                            <div class="dim" style="font-size:13px;margin-top:4px;" data-delete-message>¿Estás seguro de que querés eliminar este usuario? Esta acción no se puede deshacer.</div>
                        </div>
                    </div>
                </div>
                <div style="padding:12px 24px;border-top:1px solid var(--line);display:flex;justify-content:flex-end;gap:8px;background:var(--surface-2);border-bottom-left-radius:16px;border-bottom-right-radius:16px;">
                    <button type="button" class="btn" data-modal-close>Cancelar</button>
                    <button type="submit" class="btn btn-danger" style="background:var(--danger);color:#fff;border-color:var(--danger);">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a2 2 0 012-2h2a2 2 0 012 2v2"/></svg>
                        Eliminar
                    </button>
                </div>
